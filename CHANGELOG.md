...
Floor bumped to `cloakllm[attestation,timestamping,detection]>=0.12.1,<0.13.0` to pull the SDK's v0.12.1 detection fix (spaced/formatted phone numbers — e.g. French `06 12 34 56 78` — no longer leak on the default config). No `server.py` change; 14 tools unchanged. Re-aligned to 0.12.1 with py/js. Tests unchanged (150).
...
Floor bumped to `cloakllm[attestation,timestamping]>=0.11.3,<0.12.0` so a fresh MCP install pulls the v0.11.3 NER resilience fix (a broken/absent spaCy degrades to regex-only instead of crashing sanitize). No server.py change. Re-aligned to 0.11.3 with py/js. Tests unchanged (150).

## [0.11.2] - 2026-06-23

Floor bumped to `cloakllm[attestation,timestamping]>=0.11.2,<0.12.0` so a fresh MCP install pulls the v0.11.2 detection hardening (spaced credit-card / IBAN partial-leak fixes + IPv6 detection). No server.py change. Re-aligned to 0.11.2 with py/js. Tests unchanged (150).

## [0.11.1] - 2026-06-23

Floor bumped to `cloakllm[attestation,timestamping]>=0.11.1,<0.12.0` so a fresh MCP install pulls the v0.11.1 timestamping crypto hardening (the required ESS `SigningCertificateV2` check + the OpenSSL-differential-verified verifier). No server.py change. Re-aligned to 0.11.1 with py/js. Tests unchanged (150).
